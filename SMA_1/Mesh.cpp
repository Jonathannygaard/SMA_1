...
void Cube::CreateInvertedCube(glm::vec3 position, glm::vec3 scale, glm::vec3 color, glm::vec3 rotation)
{
	Position = position;
	Scale = scale;
	...
	/* Front */
	cIndices.emplace_back(2,1,0);
	cIndices.emplace_back(0,3,2);

	/* Right */
	cIndices.emplace_back(6,5,1);
	cIndices.emplace_back(1,2,6);

	/* Left */			   
	cIndices.emplace_back(0,4,7);
	cIndices.emplace_back(7,3,0);
						   
	/* Back */
	cIndices.emplace_back(4,5,6);
	cIndices.emplace_back(6,7,4);

	/* Top */
	cIndices.emplace_back(3,7,6);
	cIndices.emplace_back(6,2,3);

	/* Bottom */
	cIndices.emplace_back(0,1,5);
	cIndices.emplace_back(5,4,0);
...
void Cube::ChasePlayer(Cube& Player)
{
	glm::vec3 Direction = Player.Position - Position;
	Position += glm::normalize(Direction) * NPCSpeed;
}
...
{
	return glm::perlin(glm::vec2(t, u));
	//return cos(t) + sin(u);
}
...
void Mesh::CreateTerrain(float xStart, float yStart, float xEnd, float yEnd, float Step)
{
	...
	for (float i = xStart; i < xEnd; i += Step)
	{
		...
		for (float j = yStart; j < yEnd; j += Step)
		{
			...
			mVertices.emplace_back(glm::vec3(i*2, xy(i, j), j*2), glm::vec3(r,g,b));
		}
	}
	float index = 1;
